  ...
  routeConfigs.forEach(function(route, index) {
    ...
    route.handler = async function(request, h) {
      ...
      try {
        ...
        // Compatibility shim: reply.redirect(url) for Hapi 20+. Several controllers
        // (pages.login/welcome/home/signup) call reply.redirect() directly — a
        // Hapi 4.x/17.x pattern that isn't native to the Hapi 20 toolkit here — so
        // provide it, routing the response through responseResolver like the others.
        reply.redirect = function(url) {
          var response = h.redirect(url);
          responseResolver(response);
          return response;
        };
        ...
        if (err) {
          // If it's already a Boom HTTP error (e.g. Boom.forbidden()), pass it through
          // unchanged so the correct HTTP status is returned to the client.
          if (err.isBoom) {
            return err;
          }
  ...
  for (var static in config.app.prefixes) {
    if (config.app.prefixes[static]) {
      ...
      routes.push({
        ...
        handler: {
          directory: {
            // {path*} excludes the /<static>/ segment, so the directory must
            // include it — a bare './public' resolved /prefix/js/foo.js to
            // ./public/foo.js and 404'd every prefixed asset (ENG-2239).
            path: './public/' + static,
            redirectToSlash: true
          }
        }
